/*
    ...
    and treat letters as case insensitive. If a letter appears more than
    ...
    (ignoring case), and 'c' twice. Possible completing words are "abccdef", "caaacab", and "cbca".
...
*/
// APPROACH: Create a map to store all the letters in license plate
// and therir count convert the letters to lowercase
// Loop through words and add each letter into a map with their frequency
// Loop through each letter in each word and return when their is one 
// with all the letters found in the licence plate
string shortestCompletingWord(string licensePlate, vector<string>& words) {
        ...
        for (string word: words) {
            ...
            for (auto vals: lpFreq) {
                cout << vals.first << "\n";
                cout << vals.second << "\n";

                if (cwFreq[vals.first] < vals.second ) found = 0;
            }

            if (found == 1 && word.size() <s) {
                res = word;
                s = word.size();
            }

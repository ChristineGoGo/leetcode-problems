...
#include <map>
using namespace std;
// APPROACH: Create a map to map the letters to their ascii values,
// variable for the score of ascii
// loop through s calculating the score and return it

int scoreOfString(string s) {
    int score = 0, n = s.length();

    for (int i = 0; i < n - 1; i++){
        int currSum = s[i] - s[i + 1] < 0 ? (s[i] - s[i + 1]) * - 1: s[i] - s[i + 1];
        score += currSum;
    }
...
int main() {
    cout << "hello score: " << scoreOfString("hello");
    return 0;
}

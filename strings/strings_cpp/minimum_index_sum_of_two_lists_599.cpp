...
using namespace std;
    ...
    vector<string> findRestaurant(vector<string>& list1, vector<string>& list2) {
        ...
        map<string, int> commonStrings;

        for (int i = 0; i < list1.size(); i++) {
            for (int j = 0; j < list2.size(); j++) {
                if (list1[i] == list2[j]) {
                    int curr = j + i;
                    commonStrings[list1[i]] = curr;
                }
            }
        }

        for (auto s: commonStrings ) {
    ...
    int main( ) {
        vector<string> list1 = {"S","TEXP","BK","KFC"};
        vector<string> list2 = {"KFC","BK","S"};

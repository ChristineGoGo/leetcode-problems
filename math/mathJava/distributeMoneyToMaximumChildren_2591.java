...
public class distributeMoneyToMaximumChildren_2591 {
    public int distMoney(int money, int children) {
        /*
        * APPROACH: Assign all children $1 according
        * to the requirement. Next find out how many
        * children will get $7 more to have a total
        * of $8. After that, modifiy if there is any
        * money remaining or there is a child with $3
        * according to the requirements.
        */
        int remMoney = money - children;
        int remChild = children;
        int maxChildren = 0;

        if (money < children) return -1;

        while (remMoney >= 7 && remChild > 0) {
            remMoney -= 7;
            maxChildren++;
            remChild--;
        }
        ...
        if (maxChildren == 0) {
            ...
            if (remChild == 0 && remMoney > 0) {
                maxChildren--;
            }
            if (remChild == 1 && remMoney == 3) {
                maxChildren--;
            }
        }

...
/*
 * APPROACH: Use a hashmap to track the frequency of each number
 * store the numbers with a frequency of more than one in the results
    ...
    public static int[] getSneakyNumbers(int[] nums) {
        ...
        for(int i =0; i < n; i++) {
            if (m.get(nums[i]) == null) {
                ...
                current = m.get(nums[i]);
                current++;
                m.put(nums[i], current);
            }
            if (m.get(nums[i]) > 1) {

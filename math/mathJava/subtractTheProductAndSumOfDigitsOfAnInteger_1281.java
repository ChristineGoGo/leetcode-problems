/*

    Given an integer number n, return the difference between the product of its digits and 
    the sum of its digits.
 

    Example 1:

    Input: n = 234
    Output: 15 
    Explanation: 
    Product of digits = 2 * 3 * 4 = 24 
    Sum of digits = 2 + 3 + 4 = 9 
    Result = 24 - 9 = 15
    Example 2:

    Input: n = 4421
    Output: 21
    Explanation: 
    Product of digits = 4 * 4 * 2 * 1 = 32 
    Sum of digits = 4 + 4 + 2 + 1 = 11 
    Result = 32 - 11 = 21
    

    Constraints:

    1 <= n <= 10^5
*/

/*
 * APPROACH: 
 * 1.Convert the given number into a string
 * 2.Assign the product and the sum variables
 * 3.Loop through the string to find the product and the sum
 * 4.Find the difference between the product and the sum
 */

public class subtractTheProductAndSumOfDigitsOfAnInteger_1281 {
    public static int subtractProductAndSum(int n) {
        String sn = Integer.toString(n);
        int product = 1;
        int sum = 0;
        

        for (int i = 0; i < sn.length(); i++) {
            String s = "";
            int current = Integer.valueOf(s + sn.charAt(i)); 
            product *= current;
            sum += current;
        }
        return product - sum;
    }

    public static void main(String[] args) {
        System.out.println(subtractProductAndSum(15));
    }


    
}

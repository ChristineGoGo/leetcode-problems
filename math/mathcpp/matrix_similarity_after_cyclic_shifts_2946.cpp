/*
    You are given an m x n integer matrix mat and an 
    integer k. The matrix rows are 0-indexed.
...
bool areSimilar(vector<vector<int>>& mat, int k) {
    ...
    // the rows are the same, just look at the columns
    // the shifts are reduced to k % n
    // after n shifts the matrix is similar to initial matrix
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {

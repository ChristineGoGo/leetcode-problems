/*
    ...
    The subarrays of length between l = 2 and r = 3 where the sum is greater than 0
    are:
...
// APPROACH: Use the sliding window approach to find all the sums
// of the subarray and return the minimum
int minimumSumSubarray(vector<int>& nums, int l, int r) {
    ...
    for (int start = l; start <= r; start++) {
        ...
        minSum = currSum > 0 ? min(currSum, minSum) : minSum;
        ...
        for (int end = start; end < n; end++) {
            endSum += (nums[end] - nums[end - start]);
            minSum = endSum > 0 ? min(endSum, minSum) : minSum;
        }
    }

    return minSum != 100000 ? minSum : -1;

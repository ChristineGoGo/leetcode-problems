...
// size 1 plus the size of encoded(t)
// Populate the vector with the number we are given
// Loop through each index 0 to t
//      - for each index arr[idx + 1] = encoded[idx] XOR arr[idx]
vector<int> decode(vector<int>& encoded, int first) {
    int t = encoded.size();
    vector<int> arr(t + 1, 0);
    arr[0] = first;
    for (int idx = 0; idx < t; idx++) {
        arr[idx+1] = encoded[idx] ^ arr[idx];
    }

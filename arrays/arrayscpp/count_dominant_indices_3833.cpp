/*
    ...
    An element at index i is called dominant if: nums[i] > average(nums[i + 1], nums[i + 2],
    ..., nums[n - 1])
...
// APPROACH: Find the sum of all the numbers in nums
// Find total items in nums. For each index in nums
// except the right most. Find average (totalSum - nums[i] / total numbers - index - 11)
// totalSum - nums[i]. If totalSum is greater than average add to the dominant

int dominantIndices(vector<int>& nums) {
    int totalSum = 0; int n = nums.size(); int totDom = 0;
    ...
    for (int i = 0; i < n - 1; i++) {
        int curr = totalSum - nums[i];
        double average = curr / (n - i  - 1);
        totalSum = curr;
        if (nums[i] > average) totDom++;
    }

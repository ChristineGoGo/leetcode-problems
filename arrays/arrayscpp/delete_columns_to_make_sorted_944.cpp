/*
    ...
    daf
    ghi
    Columns 0 and 2 are sorted, but column 1 is not, so you
    ...
    a
    b
    Column 0 is the only column and is sorted, so you will
    not delete any columns.
    Example 3:
...
int minDeletionSize(vector<string>& strs) {
        // create variables to store the rows, columns
        int n = strs[0].size();
        int m = strs.size();
        int dCol = 0;

        // loop through comparing  the columns that are sorted lexicographically
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m - 1; j++ ) {

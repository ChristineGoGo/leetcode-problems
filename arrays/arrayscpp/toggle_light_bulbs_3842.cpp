/*
    ...
    Return the list of integers denoting the light bulbs that are on
    in the end, sorted in ascending order. If no bulb is on, return an empty list.
...
// APPROACH: Create a map to store the bulbs and their
// states ON/OFF. Create array to store the results
// Populate the map giving the map ON/OFF according to the conditions
vector<int> toggleLightBulbs(vector<int>& bulbs) {
    ...
    for (auto vals: states) {
        if(vals.second == "on") res.push_back(vals.first);
    }

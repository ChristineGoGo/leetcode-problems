/*
    ...
    Note: Since nums is circular, moving past the last element wraps around to the beginning, and moving before
    the first element wraps back to the end.
...
// APPROACH: Create variable equal to length of nums
// Create vector to store the  result array
// Loop for each index in nums satisfying requirement and
// populating the result array
vector<int> constructTransformedArray(vector<int>& nums) {
    int n = nums.size(); int resultIndex = 0;
    vector<int> result(n,0);

    for(int i = 0; i < n; i++) {
        int rIdx = ((i + nums[i]) % n + n) % n;
        result[i] = nums[rIdx];
    }

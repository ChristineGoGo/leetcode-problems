/*
    ...
    Each events[i] = [indexi, timei] indicates that the button at index index i was pressed at time timei.
...
*/
// APPROACH: Initialize a variable to store the longest press
// time. Loop for each event to find the longest press time and the
// corresponding time
int buttonWithLongestTime(vector<vector<int>>& events) {
    int longestTime = events[0][1]; int buttonIndex = events[0][0];
    int start = longestTime;
    int n = events.size();

    for (int i = 1; i < n; i++) {
        int currTime = events[i][1] - start;
        if (currTime == longestTime && events[i][0] < buttonIndex) {

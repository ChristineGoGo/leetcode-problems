/*
    ...
    0 <= nums[i] <= 1000
*/
// APPROACH: Create an array filled with zeros to store results
// Loop for each index in nums
// If number at current index is equal to the next
//      - double it and set next number to a 0
// Loop through modified nums add all the non zeros to results and return
// Keep tabs of the index that we are putting zeros into

vector<int> applyOperations(vector<int>& nums) {
    int n = nums.size();
    vector<int> results(n, 0);
    // find duplicates and double them
    for (int i = 0; i < n - 1; i++) {
        if (nums[i] == nums[i + 1]) {
            nums[i] *= 2;
            nums[i + 1] = 0;
    ...
    int currentIndex = 0;
    // move zeros to the end of array to get results
    for (int i = 0; i < n; i++) {
        if (nums[i] != 0) {
            results[currentIndex] = nums[i];
            currentIndex++;
        }
    }

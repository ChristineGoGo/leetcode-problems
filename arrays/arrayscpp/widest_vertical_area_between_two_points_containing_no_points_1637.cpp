...
// APPROACH: Find the widest area between two x coordinates
// Put all the points[0][y] into a vector. Sort the vector. Return
...
int maxWidthOfVerticalArea(vector<vector<int>>& points) {
    ...
    stable_sort(xs.begin(), xs.end());

    int mx = 0;
    for (int i = 0; i < n - 1; i++) {
        int curr = xs[i + 1] - xs[i];
        if (curr > 0){
            mx = max (curr, mx);
        }
    }

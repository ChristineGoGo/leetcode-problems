/*
    ...
    having length n, denoting the battery percentages of n 0-indexed devices.
...
// APPROACH: Loop through the batteryPercentages[i = 0 to i = n -1],
// n is the length of batteryPercentages. Increment total tested devices
//  if batteryPercentages[i] > 0,  and decrease batteryPercentages[j] by
//  1 from [j = i + 1 to
// j = n - 1] if batteryPercentages[j] > 0
//  return the total tested devices

int countTestedDevices(vector<int>& batteryPercentages) {
    int testDev = 0, n = batteryPercentages.size();

    for (int i = 0; i < n; i++ ) {
        if (batteryPercentages[i] > 0) {
            testDev++;
            for (int j = i + 1; j < n; j++) {
                batteryPercentages[j] = batteryPercentages[j] > 0 ? --batteryPercentages[j] : 0;
            }
        }

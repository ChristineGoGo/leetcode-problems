...
public class relativeRanks_506 {
    public String[] findRelativeRanks(int[] score) {
        //sort the athletes according to scores
        int n = score.length;
        int[] cScore = Arrays.copyOf(score, n );
        Arrays.sort(cScore);
        ...
        // loop to find the rankings of each athlete
        for(int i = n - 1; i >= 0;i--) {
            if (start <= 3){
                
                switch(start) {
                    case 1 -> rankings.put(cScore[i], "Gold Medal");
                    case 2 -> rankings.put(cScore[i], "Silver Medal");
                    case 3 -> rankings.put(cScore[i], "Bronze Medal");
                }
                
            } else {
                rankings.put(cScore[i], Integer.toString(start));
            }
            start++;
        }
        // System.out.println(rankings.values());
        // System.out.println(rankings.keySet());

        //populate the results
        for (int i = 0; i < score.length; i++) {
            results[i] = rankings.get(score[i]);
        }

...
public class destinationCity_1436 {
    //APPROACH: Use two lists to store the final destinations and 
    // next destinations. Return the lone destination.
    public String destCity(List<List<String>> paths) {
        List<String> nDestinations = new ArrayList<>();
        List<String> fDestinations = new ArrayList<>();

        for (List<String> path: paths) {
            nDestinations.add(path.get(0));
            fDestinations.add(path.get(1));
    
        }

        for (String location: fDestinations) {
            if (!nDestinations.contains(location)) {
                return location;
            } 
        }
        return null;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(s.contains("Cairo"));
    }
